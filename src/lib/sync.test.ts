...
describe('pull', () => {
  ...
  it('讀得回來就回 ok，網址帶密鑰', async () => {
    ...
    if (result.status === 'ok') expect(result.record.state.courses).toHaveLength(15)
...
describe('push', () => {
  ...
  it('送出去的內容包含完整的 state', async () => {
    ...
    expect(body.state.courses).toHaveLength(15)
...
describe('describeState', () => {
  it('講清楚兩邊差在哪', () => {
    ...
    expect(describeState(state)).toBe('15 門課、2 筆待辦（1 筆未完成）、0 筆重要日期')
  })
})
