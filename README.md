...
「學校行事曆」分頁可以登記學校的重要日期：開學日、期中考週、期末考週、國定假日、補課日。
...
所以這裡採手動登記。日期請以銘傳官方公告為準——這個專案不內建任何預設的學校日期。
...
> 匯出前請到「學校行事曆」分頁改成正確日期，否則課程會重複到錯的週次。
> UI 上會一直提示到你改掉為止。
...
- `version` 欄位用來做 schema migration，改結構時在 `src/lib/storage.ts` 的
  ...
  目前是 **v2**（v1 -> v2 加了 `schoolEvents`，舊資料會自動補成空陣列，不會掉東西）
...
src/
  ...
  lib/
    ...
    schoolCalendar.ts   學校行事曆的區間、進行中判斷、近期篩選
    conflicts.ts        衝堂偵測
    ics.ts              .ics 產生器
...
遇到補課或停課，請到「學校行事曆」登記一筆，首頁就會提醒你，匯出的行事曆裡也會有。
